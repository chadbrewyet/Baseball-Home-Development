...
document.querySelector("#signup-form").onsubmit=async e=>{e.preventDefault();const username=document.querySelector("#signup-username").value.trim(),name=document.querySelector("#signup-name").value.trim(),pin=document.querySelector("#signup-pin").value;if(users.some(u=>(u.username||u.name).toLowerCase()===username.toLowerCase())){alert("That username is already taken.");return}const h=await ClubhouseDB.hashPin(pin),user={id:ClubhouseDB.id("user"),username,name,pinSalt:h.salt,pinHash:h.hash,active:true,status:"pending_association",roles:[],loginCount:0,lastLoginAt:null};await ClubhouseDB.put("users",user);document.querySelector("#signup-dialog").close();document.querySelector("#signup-form").reset();await refreshRecords();loginScreen("Account created. Log in now; access is limited to Account Management.")};
