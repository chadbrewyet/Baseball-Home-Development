...
function renderAdmin(){
  ...
  if(isUnassociated()){
    ...
    document.querySelector("#profile-list").innerHTML=`<div class="manage-row"><div><strong>${currentUser.name}</strong><small>${currentUser.username||currentUser.name} · Account created · Requested ${currentUser.requestedRole||"access"}</small></div></div>`;
    ...
    document.querySelector("#request-list").innerHTML=accessRequests.filter(r=>r.userId===currentUser.id).map(r=>`<div class="manage-row"><div><strong>${r.requestedRole}</strong><small>${r.status}</small></div></div>`).join("")||`<div class="empty-state">No access request recorded.</div>`;
    return;
  }
